...
testCase('People', {
   ...
   * DOCS: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter
   */
  'Filter the list and keep those older than 35 and pick just their names':
    function () {
      var people = [
        {
          name: 'Frank',
      ...
      assert(
        namesOfThoseOlderThan35.join(', ') == ['Frank', 'Hank'].join(', ')
      );
    },
});
...
testCase('Flights', {
  'Map into destinations': function () {
    ...
    assert(
      destinations.join(', ') ==
        'San Francisco, New York, San Diego, Los Angeles'
    );
  },
  ...
  'Filter out (remove) those that cost less than 5k': function () {
    ...
    assert(
      costMoreThan5k.every(function (flight) {
        return flight.price > 5000;
      })
    );
  },
...
var todos = [
  ...
  {
    id: 'e',
    content: 'Read that article about framework X that will save the world..',
    completed: false,
  },
];
...
testCase('Todos', {
  ...
  'Sum of incomplete todos': function () {
     ...
     * Fill in necessary statement(s) to pass the assertions below
     */
    var sumIncompleteTodos = todos.filter(function (todo) {
      return !todo.completed;
    }).length;
